 ...
 *
 * The avatar is a 1024x1024 headshot with a lot of empty backdrop around it.
 * At 16px that backdrop is most of the icon and the face turns to mush, so
 * everything below is cut from a tighter square around the head first.
 */
...
/** The cropped head, resized to `size`, as PNG bytes. */
const render = (size) =>
  sharp(source)
    .extract(CROP)
...
const files = [
  ...
  // and opaque — a transparent one comes out with a black backing.
